...
public class ExtFlightDelaysDAO {
	...
	public List<Airport> getVertici(int x,Map<Integer,Airport>idMap){
		String sql ="SELECT a.id FROM airports a, flights f WHERE (a.id = f.ORIGIN_AIRPORT_ID OR a.id = f.DESTINATION_AIRPORT_ID) GROUP BY a.id HAVING COUNT(DISTINCT(f.AIRLINE_ID)) >? ";
	...
	public List<Rotta> getRotte(Map<Integer,Airport>idMap) {
		String sql = "SELECT f.ORIGIN_AIRPORT_ID AS a1 , f.DESTINATION_AIRPORT_ID AS a2, COUNT(*) AS n FROM flights f GROUP BY f.ORIGIN_AIRPORT_ID , f.DESTINATION_AIRPORT_ID";
